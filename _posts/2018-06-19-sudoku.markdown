...
>原本看到`next_bt(r, c)`这种只有一两行的 funcion 想说加上 [inline](http://en.cppreference.com/w/cpp/language/inline) 减少 call function 的 overhead 来增快速度，实测上也确实有微小的增快，而 GNU 自带的 [optimization flag](https://gcc.gnu.org/onlinedocs/gcc/Optimize-Options.html) 增快的幅度是以倍数来计算的！加上 inline 的优化后反而慢了一点……所以如果可以自己 compile 的话或许就不用考虑用 inline 来加速吧。
...
另外，本法相较与 backtracking 的绝对优势就是，它可以快速发现当下数独无解！
即每次填入数字都是确定的，那么在这种假设下，若有任一空白处无法填入任何数字，则表示无解。
这部分在主体中的：
...
解法三中，影印的比喻是为了配合程式实作上的便利，我想实际上一般人不会这么做。通常（在无法推理又想解出来时）大家会先用原子笔填入确定解，不确定的部分改用铅笔填写，发现错了就把用铅笔填的擦掉。（用铅笔之后又遇到瓶颈时就想象用更浅的铅笔来填）
...
也就是说，我们除了以**格**的角度（某空白处只能填一个数字）来寻解之外，亦可以以**行**、**列**、**宫**的角度来观察。
...
每次执行 100×n 回，去除较大较小取其中 60 轮的时间计算平均。（n 随各题计算时间长短做调整，少则1，多则1000）
