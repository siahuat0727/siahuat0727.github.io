...
[gif 来源点击这里](https://en.wikipedia.org/wiki/Backtracking#/media/File:Sudoku_solved_by_bactracking.gif)
...
```
*`can_fill_row(r, n)`寻找第 `r` raw是否已存在数字`n`，以此类推。*
*`box`表示数独中的宫 (小 3x3)。*
...
解法一、二虽然可行，但每一步都用猜的，具不确定性，而很多时候有些空白处就只能填一个数字。（是確定的）
...
这种做法实际上不会比较快（维护 stack 所需的时间成本较高），但就算一直遇到瓶颈也只会存在一份数独。
...
一个数字确定为某格子的解，除了该格子就只能填一个数字之外，还有其他情况，比如在某一**行**中，某个数字就只能填在该行某一空白处时，则该数字为该空白处之解。
同理，以**列**和**宫**的角度也一样。

也就是说，我们除了以**格**的角度（某格是否只能填一个数字）来寻解之外，亦可以以**行**、**列**、**宫**的角度来观察。

如下图所示，每个观察角度的求解能力（可得出之**解集合**）是不同的：

![](https://i.imgur.com/6mJ0M6H.png)

*图中 row 集合表示以 row 的角度来观察可得出之解集合*
*如 * 位于可由 **cell** 或 **row**、不可由 **col** 或 **box** 观察得出之解集合*
*[Venn diagram](https://en.wikipedia.org/wiki/Venn_diagram) with [n = 4](https://en.wikipedia.org/wiki/File:Venn%27s_four_ellipse_construction.svg)*

用越多角度来观察越有机会降低试错的次数，但这也造成一定的 overhead，具体哪种解法比较快因数独难度而异。
...
再次强调，上述四种角度，单用任何一种或是组合使用都可以在有限时间内求解简单数独。

当然我们也可以很容易设计出有利于特定角度的数独，比如下图的数独在只有在有**行**的角度观察时可马上得知无解，反之则无法在可接受时间内得出结论。
